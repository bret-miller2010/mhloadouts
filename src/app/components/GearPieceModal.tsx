...
const GearPieceModal = ({ gearPiece, handleUpdate }: { gearPiece: ArmorObject; handleUpdate: () => void }) => {
    return (
        <div className="flex gap-2">
            <div
                onClick={handleUpdate}
                className="w-100 h-28 bg-gray-700 rounded-md flex justify-between items-center p-2 hover:bg-gray-600 cursor-pointer">
            ...
            <div className="w-100 h-28 flex flex-col gap-1">
                {gearPiece.slots.map((slot, index) => {
                    return (
                        <div
                            key={index}
                            className="w-full h-[30px] bg-gray-700 rounded-md flex items-center pl-1">
                            <Image
                                src={`/images/${slot}.png`}
                                width={20}
                                height={20}
